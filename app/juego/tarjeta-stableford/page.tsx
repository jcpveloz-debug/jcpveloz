...
// puntos Stableford a partir del neto vs par
// neto = gross - ventaja - par.  +2 o peor=0, +1=1, 0=2, -1=3, -2=4, -3=5...
...
function colorPuntos(p: number): string {
  if (p >= 3) return '#2ECC71'   // birdie neto o mejor = verde
  if (p === 2) return '#F39C12'  // par neto = naranja
  if (p === 1) return '#81c784'  // bogey neto = verde suave
  return '#4a7a50'               // 0 puntos = apagado
}
...
export default function TarjetaStablefordPage() {
  ...
  // puntos de un hoyo (null si no hay gross)
  function puntosHoyo(j: Jugador, h: Hoyo): number | null {
    const g = getScore(j.id, h.hole_number)
    if (g === '') return null
    const neto = Number(g) - ventajaEnHoyo(j.hcp, h.si) - h.par
    return puntosStableford(neto)
  }
  ...
  function puntosTramo(j: Jugador, tr: Hoyo[]): number {
    let s = 0
    tr.forEach(h => { const p = puntosHoyo(j, h); if (p !== null) s += p })
    return s
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0a1a0f', fontFamily: 'Georgia, serif', color: '#e8f5e9' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1a3a1f 0%, #0d2410 100%)', borderBottom: '2px solid #2ECC71', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 18, fontWeight: 'bold' }}>📋 Tarjeta Stableford</div>
        </div>
      ...
      <div style={{ padding: '16px 16px 80px' }}>
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 13, minWidth: 'max-content' }}>
            <thead>
              <tr>
                ...
                {tramoActual.map(h => (
                  <th key={h.hole_number} style={{ padding: '6px 0', textAlign: 'center', color: '#e8f5e9', minWidth: 46, borderLeft: '1px solid #2ECC7122' }}>{h.hole_number}</th>
                ))}
                <th style={{ padding: '6px 10px', textAlign: 'center', color: '#2ECC71', fontWeight: 'bold', borderLeft: '1px solid #2ECC7122' }}>{tramo === 'front' ? 'FRONT' : 'BACK'}</th>
              </tr>
              <tr>
                ...
                {tramoActual.map(h => (<td key={h.hole_number} style={{ padding: '2px 0', textAlign: 'center', color: '#4a7a50', fontSize: 10, borderLeft: '1px solid #2ECC7122' }}>{h.par}</td>))}
              ...
              <tr>
                ...
                {tramoActual.map(h => (<td key={h.hole_number} style={{ padding: '2px 0', textAlign: 'center', color: '#4a7a50', fontSize: 10, borderLeft: '1px solid #2ECC7122' }}>{h.si}</td>))}
                <td style={{ padding: '2px 10px' }}></td>
              </tr>
            </thead>
            <tbody>
              {jugadores.map((j, idx) => {
                const ptsTr = puntosTramo(j, tramoActual)
                return (
                  <tr key={j.id} style={{ background: idx % 2 === 0 ? '#162a1a' : 'transparent' }}>
                      ...
                      return (
                        <td key={h.hole_number} style={{ padding: '4px 0', textAlign: 'center', borderLeft: '1px solid #2ECC7122' }}>
                          <div onClick={() => abrirPanel(j.id, h.hole_number)} style={{
                            width: 44, minHeight: 40, margin: '0 auto', padding: '3px 0',
                            ...
                            {p !== null && (
                              <div style={{ fontSize: 9, fontWeight: 'bold', color: colorPuntos(p), lineHeight: '11px' }}>
                                {p} pt{p !== 1 ? 's' : ''}
                              </div>
                            )}
                    ...
                    <td style={{ padding: '4px 10px', textAlign: 'center', fontWeight: 'bold', color: '#2ECC71', fontSize: 16, borderLeft: '1px solid #2ECC7122' }}>{ptsTr}</td>
                  </tr>
                )
              })}
            </tbody>
        ...
        {/* Tabla de resultados: puntos FRONT, BACK, TOTAL */}
        <div style={{ background: '#1a2e1d', borderRadius: 14, padding: '14px', border: '1px solid #2ECC7122', marginBottom: 16 }}>
          <div style={{ fontSize: 11, letterSpacing: 2, color: '#2ECC71', textTransform: 'uppercase', marginBottom: 10 }}>Resultados (puntos Stableford)</div>
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: 'max-content', width: '100%' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ padding: '6px 8px', textAlign: 'center', color: '#81c784', fontWeight: 'normal' }}>Pts F</th>
                  <th style={{ padding: '6px 8px', textAlign: 'center', color: '#81c784', fontWeight: 'normal' }}>Pts B</th>
                  <th style={{ padding: '6px 8px', textAlign: 'center', color: '#2ECC71', fontWeight: 'bold' }}>Pts Total</th>
                </tr>
              </thead>
              <tbody>
                {jugadores.map((j, idx) => {
                  const gross = grossTramo(j.id, hoyos)
                  const pf = puntosTramo(j, front)
                  const pb = puntosTramo(j, back)
                  const pt = pf + pb
                  return (
                    <tr key={j.id} style={{ background: idx % 2 === 0 ? '#0d2410' : 'transparent' }}>
                      ...
                      <td style={{ padding: '8px', textAlign: 'center', fontWeight: 'bold' }}>{pf}</td>
                      <td style={{ padding: '8px', textAlign: 'center', fontWeight: 'bold' }}>{pb}</td>
                      <td style={{ padding: '8px', textAlign: 'center', fontWeight: 'bold', color: '#2ECC71', fontSize: 15 }}>{pt}</td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <div style={{ marginTop: 10, fontSize: 10, color: '#81c784', lineHeight: 1.5 }}>
            ...
            <div>Puntos: par neto = 2, birdie neto = 3, bogey neto = 1, doble bogey o peor = 0. Gana quien más puntos junta.</div>
          </div>
        </div>

        {/* Botón ranking */}
        <button onClick={() => window.location.href = `/juego/resumen-stableford?game=${gameId}${adminSuffix}`} style={{
